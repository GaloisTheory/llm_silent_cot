...
def _get_prefix(model: str) -> str:
    """Get folder prefix for non-baseline experiments."""
    return f'qwen_{model}'


def _get_baseline_prefix(model: str) -> Optional[str]:
    """Get folder prefix for baseline experiments. 8B baselines have no prefix."""
    if model == '8B':
        return None  # 8B baselines are just 'baseline_*'
    return f'qwen_{model}'


def _load_experiment(name: str, prefix: str) -> Optional[dict]:
    """Load an experiment by name with prefix."""
...
def load_experiments(model: str = '8B') -> dict:
    ...
    baseline_prefix = _get_baseline_prefix(model)
    
    # Load baselines (8B baselines have no prefix, others use qwen_{model})
    cot = _analyze_utils.load_baseline_combined(_batch_utils.load_batch_experiment, prefix=baseline_prefix)
    
    # Load blank static experiments (0 = NoCoT)
    blank_static = {}
    ...
    # NoCoT = blank_static_0
    nocot = blank_static.get(0)
...
def load_experiment(
    ...
    if exp_type == 'cot':
        exp = _analyze_utils.load_baseline_combined(_batch_utils.load_batch_experiment, prefix=baseline_prefix)
        if exp:
            exp['_model'] = model
            exp['_type'] = 'cot'
        return exp
        ...
        # NoCoT = blank_static_0
        exp = _load_experiment('blank_static_0', prefix)
        if exp:
            exp['_model'] = model
            exp['_type'] = 'nocot'
        return exp
...
# =============================================================================
# MATS SUMMARY TABLE (Cross-Model with Best/Worst)
# =============================================================================
...
def _count_correct(exp: dict) -> Tuple[int, int, int, int]:
    """Count correct samples and questions. Returns (q_correct, q_total, s_correct, s_total)."""
    if not exp or not exp.get('results'):
        return 0, 0, 0, 0
...
def _format_acc(acc: Optional[float], correct: int, total: int) -> str:
    """Format accuracy with counts like '83.0% (83/100)'."""
    if acc is None or total == 0:
        return '-'
    return f'{acc*100:.1f}% ({correct}/{total})'


def _find_best_worst_exp(model_data: dict, exp_type: str = 'blank') -> Tuple[Optional[dict], Optional[dict]]:
    """
    Find best and worst experiments by sample accuracy.
    
    Args:
        model_data: Experiments dict for one model
        exp_type: 'blank' or 'incorrect'
        
    Returns:
        (best_exp, worst_exp) - the experiment dicts
    """
    ...
    for n, exp in exps.items():
        if n == 0:  # Skip n=0 which is NoCoT
            continue
        acc = get_accuracy(exp, accuracy_type='sample')
        if acc is not None:
            accuracies[n] = (acc, exp)
    
    if not accuracies:
        return None, None
    
    best_n = max(accuracies.keys(), key=lambda k: accuracies[k][0])
    worst_n = min(accuracies.keys(), key=lambda k: accuracies[k][0])
    return accuracies[best_n][1], accuracies[worst_n][1]


def plot_mats_summary_table(
    save: bool = False,
    output_dir: Optional[str] = None,
) -> plt.Figure:
    """
    Plot MATS write-up summary table comparing all models.
    
    Format matches the exact layout requested:
    ...
    all_data = {model: load_experiments(model) for model in MODELS}
    
    # Row definitions: (display_name, getter_function)
    row_defs = [
        ('CoT', lambda d: d['cot']),
        ('NoCoT', lambda d: d['nocot']),
        ('Blank Best', lambda d: _find_best_worst_exp(d, 'blank')[0]),
        ('Incorrect Best', lambda d: _find_best_worst_exp(d, 'incorrect')[0]),
    ]
    
    # Build rows
    rows = []
    for row_name, getter in row_defs:
        row = [row_name]
        
        for model in MODELS:
            model_data = all_data[model]
            exp = getter(model_data)
    ...
    # Column labels matching requested format
    ...
    ax.axis('off')
    ax.set_title('BBQ Experiment Summary - All Models\nQ% = Question Accuracy (majority vote), S% = Sample Accuracy',
                 fontsize=12, fontweight='bold', pad=15)
    
    # Column widths
    col_widths = [0.12] + [0.12] * (len(col_labels) - 1)
    ...
    row_colors = {
        ...
        2: '#D9EAD3',  # Blank Best - light green
        3: '#D9EAD3',  # Incorrect Best - light green
    }
    ...
    if save:
        if output_dir is None:
            output_dir = _results_dir
        os.makedirs(output_dir, exist_ok=True)
        save_path = os.path.join(output_dir, 'mats_summary_table.png')
        plt.savefig(save_path, dpi=150, bbox_inches='tight', facecolor='white')
        print(f"✓ Saved: {save_path}")
